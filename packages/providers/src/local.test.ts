...
describe("createLocalSearchProvider", () => {
  ...
  it("writes a reusable local index cache after searching", async () => {
    ...
    try {
      ...
      const cache = JSON.parse(rawCache) as {
        ...
        entries?: Array<{ relativePath?: string; document?: { chunks?: unknown[] } }>;
      };
  ...
  it("writes local index caches without leaving temporary files", async () => {
    ...
    try {
      ...
      expect(JSON.parse(rawCache).schemaVersion).toBe(3);
  ...
  it("reports and clears local index cache state", async () => {
    ...
    try {
      ...
      expect(missingInfo.currentSchemaVersion).toBe(3);
      ...
      expect(readyInfo.currentSchemaVersion).toBe(3);
      expect(readyInfo.cacheSchemaVersion).toBe(3);
  ...
  it("reuses cached chunks when source metadata is unchanged", async () => {
    ...
    try {
      ...
      await writeFile(sourcePath, "Plain source text contains ordinary marker.", "utf8");
  ...
  it("repairs empty document ids when reusing old local index caches", async () => {
    ...
    try {
      ...
      await writeFile(sourcePath, "Cached multilingual evidence marker.", "utf8");
      const sourceStat = await stat(sourcePath);
      await writeFile(
        join(rootDir, ".sourceline", "cache", "local-index.json"),
        JSON.stringify(
  ...
  it("rebuilds unsupported local index cache schema versions", async () => {
    ...
    try {
      ...
      expect(oldInfo.currentSchemaVersion).toBe(3);
      expect(oldInfo.cacheSchemaVersion).toBe(1);
      expect(oldInfo.invalidReason).toContain("Expected version 3");
  ...
  it("repairs stale non-ASCII document ids when reusing old local index caches", async () => {
    ...
    try {
      ...
      await writeFile(firstPath, "Cached multilingual evidence marker one.", "utf8");
      await writeFile(secondPath, "Cached multilingual evidence marker two.", "utf8");
            ...
            entries: [
              {
                ...
                document: {
                  id: "md",
                  title: "Cached Non-ASCII Source One",
              ...
              },
              {
                ...
                document: {
                  id: "md",
                  title: "Cached Non-ASCII Source Two",
  ...
  it("rejects unsafe relative paths in local index caches", async () => {
    ...
    try {
      await writeFile(join(rootDir, "notes.md"), "Fresh safe cache evidence should be indexed.", "utf8");
      await writeFile(
        join(rootDir, ".sourceline", "cache", "local-index.json"),
        JSON.stringify(
          {
      ...
      expect(invalidInfo.cacheSchemaVersion).toBe(3);
  ...
  it("rejects invalid chunk metadata in local index caches", async () => {
    ...
    try {
      await writeFile(join(rootDir, "notes.md"), "Fresh chunk metadata evidence should be indexed.", "utf8");
      await writeFile(
        join(rootDir, ".sourceline", "cache", "local-index.json"),
        JSON.stringify(
          {
  ...
  it("rejects unsafe chunk ids in local index caches", async () => {
    ...
    try {
      const sourcePath = join(rootDir, "notes.md");
      await writeFile(sourcePath, "Fresh safe cache evidence should be indexed.", "utf8");
      const sourceStat = await stat(sourcePath);
      await writeFile(
        join(rootDir, ".sourceline", "cache", "local-index.json"),
        JSON.stringify(
